...
#' @export
#'
radical <- function(X, components = NCOL(X), demean = TRUE, pca_cov = c("ML", "LS-ID", "LS-DIAG", "EWMA"),
                    ...
                    Z = NULL, K = NULL, L = NULL, seed = NULL, trace = FALSE, ...)
{
    ...
    pca_cov <- match.arg(pca_cov[1], c("ML", "LS-ID", "LS-DIAG", "EWMA"))
    if (demean) {
...
fastica <- function(X, components = NCOL(X), demean = TRUE, method = "symmetric",
                    pca_cov = c("ML", "LS-ID", "LS-DIAG", "EWMA"), first_eigen = NULL, last_eigen = NULL, E = NULL, D = NULL,
